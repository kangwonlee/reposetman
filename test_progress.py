...
class TestProgress(unittest.TestCase):
    def setUp(self):
        self.msg = (
            "commit 0095423cbee960ab0e59c05bf0f1b75ea9ab5b18\n"
    ...
    def te_st_eval_repo(self):
        if os.path.exists("test_eval_repo.cfg"):
            ...
            args = {"fields": set(), "re_git_log": progress.get_regex_parse_git_log()}

            config = configparser.ConfigParser()
            config.read("test_eval_repo.cfg")

            repo = {"path": config["repo"]["path"], "name": config["repo"]["name"]}
    ...
    def te_st_proc_file(self):
        ...
        if os.path.exists(config_filename):
            ...
            raise IOError(
                "can't find config file {filename}".format(filename=config_filename)
            )
...
class TestRepoEvalPoundCounter(unittest.TestCase):
    ...
    def test_comments(self):
        ...
        with tempfile.TemporaryFile("w+t") as file_object:
            ...
            comments = set(
                evaluator.get_comments_list_from_readline(file_object.readline)
            )
...
class TestRepoEvalRunEachBase(unittest.TestCase):
    ...
    def init_test_run_each(self):
        if not os.path.exists(self.config_filename):
            ...
            config["operation"] = {"python_path": shutil.which("python")}
...
class TestRepoEvalRunEach(TestRepoEvalRunEachBase):
    def test_run_script_input(self):
        # function under test
        msgo, msge = self.e.run_script_input(
            [
                self.config["operation"]["python_path"],
                os.path.join(os.path.split(__file__)[0], "input_example.py"),
            ]
        )
            ...
            for py_file in py_file_list:
                msgo, msge = self.e.run_script_input(
                    "{python} {script}".format(
                        python=self.config["operation"]["python_path"], script=py_file
                    )
                )
    ...
    def test_search_sys_argv_assign_line_from_sys_import_argv(self):
        input_txt = "from sys import argv\n" "script = argv\n"
...
class TestRepoEvalCountOneCommitLog(unittest.TestCase):
    ...
    def test_convert_git_log_to_table(self):
        ...
        self.assertFalse(
            expected_files_set - set(result_columns),
            f"set(result_columns) = {set(result_columns)}",
        )
        self.assertFalse(
            expected_files_set - set(result_index.keys()),
    ...
    def get_git_log_commit_line(self, sha, author, email, date, subject):
        return (
            sha + f"{self.e.field_split_token}{author}"
    ...
    def test_get_commit_dict_03(self):
        ...
        subject = (
            '''"""가 print 명령어를 반복하지 않아도 되게 한다. 백슬래쉬를 사용하는 방법. (줄을 변경하게 함. 엔터효과)\\\\\\'''
        )
...
class TestFromSysImportArgv(unittest.TestCase):
    ...
    def test_get_argn(self):
        ...
        result = progress.get_argn(
            os.path.join(os.path.split(__file__)[0], "from_sys_argv_example_00.py")
        )
