...
class TestFetchAndReset(unittest.TestCase):
    def setUp(self):
        ...
        # test repositories
        self.first_repository = (
            "https://github.com/kangwonlee/test-reposetman-fetch-and-reset-00"
        )
        self.second_repository = (
            "https://github.com/kangwonlee/test-reposetman-fetch-and-reset-conflict"
        )

        self.cwd = os.getcwd()
        self.clone_destination_folder = os.path.join(
            self.temp_folder_name, "temp_test_fetch_and_reset"
        )
