...
def download_zipfile(proj_id, dest_path):
    ...
    if intermediate_zip_url:
        ...
        # download file
        if bVerbose:
            print(("download_zipfile() : retriving %s to %s" %
                   (zip_url, dest_path_fname)))
...
def get_intermediate_url(url):
    ...
    for table_item in items:
        ...
        if ".zip" in table_item:
            ...
            path_fname_list = re.findall(
                r'''<a\s.*?href="(.+?)"''', anchor_string, re.S)
...
def get_zip_url(intermediate_zip_url):
    ...
    num_zip_list = re.findall(
        r"download/(.*)\?filename=(.*)", intermediate_zip_url)
    ...
    zip_url = "http://dev.naver.com/frs/download.php/%s/%s" % (
        number, zip_fname)
