...
class TestMarkdownTableWriter(BaseTestTableWriterTable):
    ...
    def test_gen_rows_header(self):
        ...
        # second row
        self.assertEqual(
            len(expected_second_row_split),
            len(result_second_row_split),
            msg="\n"
...
class TestHtmlTableWriter(BaseTestTableWriterTable):
    ...
    def check_table_header(self, header_row):
        ...
        for expected_header, header_body in zip(
            self.column_title_list, header_body_list
        ):
...
class TestMDlinkTableWriter(BaseTestTableWriterRepoLinks):
    def setUp(self):
        super().setUp()
        self.table_writer = progress.MDlinkTableWriter(
            self.d,
            self.row_title_list,
            "test_MD_link_table",
...
class TestHtmlLinkTableWriter(BaseTestTableWriterRepoLinks):
    def setUp(self):
        super().setUp()
        self.table_writer = progress.HtmlLinkTableWriter(
            self.d,
            self.row_title_list,
            "test_HTML_link_table",
            repo_list=self.repo_dict_list,
        )
