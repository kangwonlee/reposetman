...
def main(argv):
    ...
    if os.path.exists("participant_folder_list.txt") and (
        "True" != config["operation"]["update_repo"]
    ):
    ...
    for section in sections_dict:
        ...
        for parse in url_parse_dict[section]:
            sections_dict[section]["user_ids"].append(
                os.path.splitext(
                    os.path.split(parse.path.strip("/"))[-1]  # last part of the path
                )[0][
                    id_starts_here:
                ]  # extract id
            )
...
def transpose_dict(sections_dict):
    ...
    for section in sections_dict:
        for url, user_id in zip(
            sections_dict[section]["urls"], sections_dict[section]["user_ids"]
        ):
            ...
            if not url.endswith(".git"):
                ...
                # add .git
                url = (
                    git.set_id_to_url(url, ret.config["Admin"]["id"]).strip("/")
                    + ".git"
                )
...
def init_or_update_umbrella_repos(users_dict, umbrella_folder, b_parallel=True):
    ...
    if b_parallel:
        ...
        repo_list = p.starmap(
            init_or_update_user_umbrella, gen_folder_user_dict(users_dict.items())
        )
...
def init_or_update_user_umbrella(umbrella_folder, user, section_url_dict):
    ...
    assert isinstance(
        user, str
    ), f"init_or_update_user_umbrella: type({user}) = {type(user)}"
    assert isinstance(
